...
data "aws_iam_policy_document" "policy" {
  ...
  dynamic "statement" {
    ...
    content {
      ...
      resources = ["arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/service/shared/*"
      ]
    }
  }
...
resource "aws_codebuild_project" "lambda" {
  ...
  source {
    ...
    git_clone_depth = 1
    buildspec       = var.use_docker ? file("${path.module}/docker_buildspec.yml") : "buildspec.yml"
  }
}
